...
options=(bio speech finance vision nlp)
...
for opt in "${options[@]}"; do
    for algo in "${algorithm[@]}"; do
        ...
        # capture stderr in a vara
        testout=$(python ${1}classify.py --mode test --model-file datasets/${opt}.perceptron.model --data datasets/${opt}.test --predictions-file datasets/${opt}.test.predictions 2>&1)
